...
# ====================================================================================
# [릴리즈 버전 - app_R5 개정판] 지정 폴더(PBA_MonitoringDB) 파일 저장 버전, 냉장고 원격 연동 추가
# ====================================================================================
...
# ------------------------------------------------------------------------------
# 🛠️ [기능 추가] 냉장고 설비 PC(오프라인 사내망 원격) 엑셀 데이터 추출 함수
# ------------------------------------------------------------------------------
def load_remote_fridge_data():
    """
    사내망으로 연결된 냉장고 설비 PC (IP: 192.168.1.10 가정)의 
    공유 폴더에서 가장 최신 엑셀 파일을 읽어와 데이터프레임으로 리턴합니다.
    """
    # [네트워크 가상 경로] 설비 PC의 공유폴더 명칭 설정 
    # 현장 환경에 맞게 네트워크 IP 및 공유 폴더명을 수정할 수 있습니다.
    network_folder = r"\\192.168.1.10\fridge_data"
    
    # 로컬 테스트용 및 네트워크 단선 대비 백업 경로 (네트워크 폴더 미존재 시 로컬 폴더 탐색)
    ...
    if not os.path.exists(target_folder):
        # 만약 사내망 네트워크 폴더가 연결되어 있지 않은 경우 예시 데모 데이터를 자동 생성합니다 (서버 다운 방지 예외 처리)
    ...
    try:
        ...
        if not excel_files:
            return None, "⚠️ 공유 폴더 내에 엑셀 파일이 존재하지 않습니다."
        
        # 30분 간격으로 쓰이는 파일 중 '가장 최근에 수정(저장)된 파일' 선별
        latest_file = max(excel_files, key=os.path.getmtime)
        file_name = os.path.basename(latest_file)
        
        # 파일 잠금 충돌 방지를 위해 원본을 해치지 않고 메모리에 임시 로드하여 읽기 (read-only 안전 모드)
        ...
        df['Date'] = df['DateTime'].dt.strftime('%Y-%m-%d %H:%M') # 분 단위 트렌드용
