...
# ==============================================================================
# [릴리즈 버전 - app_R4] 인두기 전용 입력 탭 분리 및 RS-232/모델직접입력 고도화 버전
# ==============================================================================

st.set_page_config(
    page_title="PBA 조립 공정 관리 시스템 (V4)",
    layout="wide",
...
# ==============================================================================
st.title("🏭 PBA 조립 공정 품질 모니터링 시스템 (Release app_R4)")
st.markdown("설비 RS-232 연동 및 각 공정 요소별 데이터베이스 통합 대시보드")
st.divider()
...
# ==============================================================================
# 🎛️ 사이드바 - [인두기 제외] 솔더 및 마스크 입력창
# ==============================================================================
st.sidebar.header("🎛️ 데이터 입력 및 이력 수정 (사이드바)")
input_category = st.sidebar.radio("작업 선택", ["1. 솔더크림 상온방치 관리 (RS-232)", "2. 메탈마스크 & 텐션 관리", "3. 솔더 Lot별 점도 관리"])

# --- 사이드바 1: RS-232 상온방치 로그 전용 (인두기 항목 완전 제거됨) ---
if input_category == "1. 솔더크림 상온방치 관리 (RS-232)":
    st.sidebar.subheader("⏳ RS-232 설비 데이터 매핑")
    target_date = st.sidebar.date_input("방치 일자 선택", datetime.now(), key="solder_date")
    date_str = target_date.strftime('%Y-%m-%d')
    
    with st.sidebar.form("solder_only_form"):
        sb_model = st.text_input("생산 모델명 직접 입력", value="SMT-PRIME-01").strip().upper()
        st.markdown("**🔌 RS-232 통신 수집 데이터 스펙**")
        sb_start = st.text_input("방치 시작 시간 (HH:MM)", value="08:00")
        sb_end = st.text_input("방치 종료 시간 (HH:MM)", value="10:15")
        
        submit_solder = st.form_submit_button("⏳ 방치 이력 저장")
    ...
    if submit_solder:
        ...
        # 기존 인두기 데이터 훼손 방지를 위해 동일 날짜가 있다면 기존 인두기 정보를 유지하며 병합
        ...
        st.sidebar.success("💾 RS-232 통신 데이터가 DB에 반영되었습니다.")
        st.rerun()
...
# --- [TAB 3] 메탈 마스크 타수 및 텐션 관리 섹션 (직접 입력 대응) ---
with tab_mask:
...
# --- [TAB 4] 인두기 정기 검사 섹션 (★입력 인터페이스를 내부로 별도 독립 구성) ---
with tab_iron:
    st.subheader("⚡ 인두기 호기별 관리 센터 (1번 ~ 25번)")
    
    # 🌟 [요구사항 반영] 탭 내부 상단에 별도로 배치된 전용 입력 양식
    with st.expander("➕ ⚙️ 현장 실시간 인두기 측정 데이터 등록/수정 양식 열기", expanded=False):
        ...
        if submit_iron_data:
            ...
            # 기존 데이터 중 날짜와 인두기 번호가 겹치는 행이 있다면 덮어쓰기(수정) 처리
            ...
            new_iron = pd.DataFrame([{
                'Date': i_date_str, 'Model_Name': "SYSTEM_CHECK", 
                'Solder_Start_Time': "00:00", 'Solder_End_Time': "00:00", 'Solder_Leave_Min': 0,
            ...
            df_main.to_csv(MAIN_DB, index=False, encoding='utf-8-sig')
            st.success(f"💾 {i_date_str} 기준 [{iron_no_select}] 인두기 데이터가 영구 저장되었습니다.")
            st.rerun()
    ...
    st.markdown("#### 📋 인두기 및 공정 종합 마스터 데이터 로그")
